...
public class AlgoAprioriClose {
	 ...
	 *               the result will be kept into memory and this method will return the
	 *               result.
	...
	public Itemsets runAlgorithm(double minsup, String input, String output) throws IOException {
		...
		Map<Integer, Integer> mapItemCount = new HashMap<>(); // to count the support of each item

		this.database = new ArrayList<>(); // the database in memory (intially empty)

		// scan the database to load it into memory and count the support of each single item at
		// the same time
		...
		this.minsupRelative = (int) Math.ceil(minsup * this.databaseSize);
			...
			for (int[] transaction : this.database) {
				...
				loopCand: for (Itemset candidate : candidatesK) {
					...
					for (int item : transaction) {
						...
						} else if (item > candidate.get(pos)) {
							continue loopCand;
						}
	...
	/**
	 * Checks if all the itemsets of size K-1 are closed by comparing them with itemsets of
	 * size K.
	 *
	 ...
	 * @throws IOException exception if error writing output file
	 */
	private void checkIfItemsetsK_1AreClosed(List<Integer> levelKm1, List<Itemset> levelK, Map<Integer, Integer> mapItemCount)
			throws IOException {
		// for each itemset of size k-1
	...
	protected boolean allSubsetsOfSizeK_1AreFrequent(int[] candidate, List<Itemset> levelK_1) {
		...
		if (comparison < 0) {
			...
			last = middle - 1; // the itemset compared is smaller than the subset is smaller according to the lexical
			// order
		} else {
	...
	public void printStats() {
		...
		// VAS:commented_out System.out.println(" Maximum memory usage : " +
